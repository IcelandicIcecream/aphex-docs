...
```ts title="src/lib/server/resolveAssetUrls.ts"
...
<Callout>
	All three share one session-scoped guard, so a page reloads or force-navigates **at most once**.
	A second failure means something is actually wrong — the origin is down, not hiccuping — and
	looping would serve the visitor worse than a broken-but-stable page.
</Callout>
