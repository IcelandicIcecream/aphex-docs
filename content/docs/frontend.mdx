...
<Callout>
	`ctx.organizationId` is present on the context returned by the `publicContext` helper above. If you
	build the context differently, pass whichever org id the read was scoped to — the asset must belong
