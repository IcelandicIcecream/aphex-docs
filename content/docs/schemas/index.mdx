...
| `hooks`       | `SchemaHooks`   | No       | Save-time transform hooks (`beforeValidate`). See [Hooks](/docs/schemas/hooks).                                     |

### Authoring styles

Both of these are valid — pick per file:

- `const post: SchemaType = { ... }` — a plain annotated object (used throughout these docs).
- `defineType({ ... })` — an optional wrapper from `@aphexcms/cms-core` that captures the exact `fields` so [hooks](/docs/schemas/hooks) get a `data` typed from the schema itself, with no generated types and no casts. Backwards compatible; use it only where you want typed hooks.
