...
<Callout type="info">
	The base template still wires auth by hand, and that layout stays fully supported — see [Wiring it
	by hand](#wiring-it-by-hand). Everything else on this page (sign-up, roles, organizations, route
	protection, auth shapes) applies to both paths, so you can adopt `@aphexcms/auth` in an existing
	project without changing any of it.
</Callout>

Three lines of `.env` cover the basics:

```bash title=".env"
AUTH_SECRET=long-random-string-change-in-production
AUTH_URL=http://localhost:5173
AUTH_TRUSTED_ORIGINS=http://localhost:5173
```

<Callout>
	`AUTH_SECRET` signs session cookies **and** API key hashes. Rotating it invalidates every session
	and key. Keep it long (32+ chars), random, and out of your repo. `BETTER_AUTH_SECRET` /
	`BETTER_AUTH_URL` are accepted as aliases for backwards compatibility — the app reads `AUTH_SECRET
	|| BETTER_AUTH_SECRET` — but new projects should use the `AUTH_*` names.
</Callout>
...
- A parent organization can also **write** to a child's documents, by ID.

<Callout type="warn">
	**A parent can modify child data.** Writes are not confined to the active organization,
	and it is worth knowing exactly how far they reach.

    The relational adapters wrap document writes in a hierarchy fallback: `updateDocDraft`
    and `deleteDocById` first try the caller's own organization, and if the document isn't
    there, they look through the caller's child organizations and retry against whichever
    one owns it. So a parent-org user who has a child's document ID edits or deletes that
    document successfully. This is deliberate — it is what lets a label's staff work on an
    artist's content without switching organizations — but "a parent can't accidentally
    modify child data" was never true.

    What the fallback does *not* do is widen anything else. A child can never reach up or
    sideways; the search only ever descends. `createDocument` is not wrapped, so new
    documents always land in the active organization. And a thrown error propagates without
    retrying — a `RevisionConflictError` means the document *was* found, so "maybe it lives
    in a child org" doesn't apply.
