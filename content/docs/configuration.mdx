...
Turns on the in-admin agent — a chat panel that can read and edit content through the same [agent tools](/docs/plugins#give-an-ai-agent-a-tool) exposed over MCP. Off unless you configure it. This section covers the config knobs; see [AI Assistant](/docs/ai-assistant) for what the panel does, the workspace bridge into the open editor, and the per-turn audit/undo trail.
