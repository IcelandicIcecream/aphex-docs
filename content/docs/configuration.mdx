...
<TypeTable
	type={{
		...
		allowedMimeTypes: {
			type: 'string[]',
			default: 'Built-in safelist',
			description:
				'Optional installation-wide MIME allow-list. Supports exact MIME types and wildcards; filename extensions are rejected.'
		},
		direct: {
			type: 'boolean',
			default: 'false',
...
schema field's [`accept`](/schemas/file#accepted-file-types) rule is applied in addition to this
...
```ts title="src/routes/(protected)/admin/+layout.svelte"
...
### Where plugin tools land

A plugin admin tool with `placement: 'sidebar'` renders in a group labelled **Tools**. Give the tool a `group` matching one of your `navGroups` ids to file it under your own heading instead — see [Add your own screen to the admin](/plugins#add-your-own-screen-to-the-admin). A tool naming a group you haven't defined falls back to **Tools** rather than disappearing.
...
[Private assets](/storage#private-assets) for the full flow. Without this secret, signing is a
no-op and verification always fails, so private assets stay reachable only with a session.

## aiProvider

Turns on the in-admin agent — a chat panel that can read and edit content through the same [agent tools](/plugins#give-an-ai-agent-a-tool) exposed over MCP. Off unless you configure it. This section covers the config knobs; see [AI Assistant](/ai-assistant) for what the panel does, the workspace bridge into the open editor, and the per-turn audit/undo trail.
...
<Callout>
	...
	guard](/local-api#concurrency--expectedrevision) as a human edit — an agent can't write shapes the
	admin UI couldn't, and can't silently clobber a document someone else is editing. Mutating tool
	calls are recorded per turn and can be undone from the Activity view.
</Callout>
