...
<Callout>
	**You normally don't run this by hand.** The `aphex()` Vite plugin regenerates
	`generated-types.ts` on schema save, you commit it, and builds/CI/prod use the committed file — so
	types just stay in sync. The CLI below is what the plugin calls under the hood; you'd invoke it
	directly only for edge cases (catching up after a pull without starting dev, or a CI drift-check).
</Callout>

## Running the generator

```bash
pnpm aphex generate:types [schema-path] [output-path] [plugins-path]
```
...
| `plugins-path` | — (optional; see below)          |

### Plugin-contributed collections

A plugin can contribute schemas of its own — `@aphexcms/plugin-forms` adds `form` and
`formSubmission` — and those collections are **not** in your `schemaTypes` array. Without
the third argument they're missing from the generated types, so `collections.form` doesn't
type-check even though it exists at runtime.

Pass your plugin registry and they're included:

```bash
aphex generate:types ./src/lib/schemaTypes/index.ts ./src/lib/generated-types.ts ./src/lib/plugins.ts
```

The file must export a `plugins` array and be **client-safe** — it is imported to read the
plugin definitions, so anything with a server-only import at module scope will fail here.
That's why the templates keep `src/lib/plugins.ts` separate from `aphex.config.ts`.

<Callout type="info">
	Both templates already wire the third argument. If you're upgrading a project scaffolded before
	plugins existed, add it to your `generate:types` script — a plugin's collections will otherwise
	stay untyped no matter how many times you regenerate.
</Callout>

Reached by name at runtime either way, since the typed map is generated from your app's
schemas:
