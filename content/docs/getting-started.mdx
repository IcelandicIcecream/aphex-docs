...
- **`static/uploads/`** — local fallback for image/file uploads when no `R2_*` env vars
  are set. Override with `APHEX_UPLOADS_DIR`; in a container it must point at a mounted
