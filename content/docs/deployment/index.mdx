...
Both adapters run the same cross-dialect conformance suite, so neither is missing
features — but that is a statement about capability, and production is an operational
question. The two come apart there:
...
| Backups  | A file you copy yourself, or [Litestream](https://litestream.io) you set up                   | Point-in-time recovery from any managed provider |
...
**Pick SQLite** to try Aphex, for an internal tool, or for a site where a few seconds
of downtime on deploy costs nothing. It is genuinely less to run and less to back up.

**Pick Postgres for a site you would be paged about.** Not because you will outgrow
SQLite's capacity — most sites never would — but because zero-downtime deploys and
real point-in-time recovery are the things you miss at the worst possible moment.

Switching later is one environment variable and no code change, so this is a
reversible decision — but move media to a bucket **before the first upload**, because
switching storage does not relocate files already written to disk.

### The two configurations

<Tabs items={['SQLite on a volume', 'Postgres + S3']}>

<Tab value="SQLite on a volume">
...
All four `R2_*` credentials must be set together — with any one missing the app
...
`APHEX_EMBEDDED_WORKER` off — see [Operations](/deployment/operations#background-jobs).

</Tab>

</Tabs>

Both one-click paths ship a Postgres variant:
[Render](/deployment/render#postgres-instead) has a second blueprint,
[Railway](/deployment/railway#using-railway-postgres-instead) is two clicks and two
variables. Details on either adapter: [Database](/database).
...
Protocol and host exactly, **no trailing slash, no path**. Do this even though the
...
<Callout type="warn">
	**`AUTH_TRUSTED_ORIGINS` replaces the default, it does not extend it.** It reads
    ...
    Only set it when another origin calls this app's auth routes — a separate frontend, or an apex
...
The proxy still has to forward `Host` and the original protocol for logging and
...
Set it before the first boot. Once the content exists, turning it off doesn't remove
it — you delete the documents in the admin. Keep the seed only if you're deploying a
demo on purpose.
...
| `APHEX_UPLOADS_DIR`     | `./uploads`           | Local storage path. Volume, or use `R2_*` instead.                      |
