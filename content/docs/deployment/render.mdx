...
and generates the secrets that need generating. The templates ship one, so this is
close to a single click.
...
Deploying the website template instead? Same button, different repo:
[aphex-website](https://render.com/deploy?repo=https://github.com/IcelandicIcecream/aphex-website).
Deploying your own project, scaffolded from either? Push it to GitHub and point Render
at your repo — the `render.yaml` came with the template.
