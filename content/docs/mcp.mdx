...
## Editors get the same tools in the admin

MCP is the developer-facing door. The same tool set is also available to editors as a chat panel inside the admin — with one addition MCP can't have: a live bridge into the document currently open in the editor, so the agent patches the draft in front of you rather than the database behind it. See [AI Assistant](/docs/ai-assistant).
