...
Leave the `R2_*` vars empty in `.env`. Files land in `./uploads/` and are served at `/media/{id}/{filename}`, the route that enforces access control. Restart not required after upload.
