...
So a template edit reaches users in three hops, and the last one is the part that
catches people out:

1. Merge the change — `sync-template.yml` mirrors `templates/*` to `aphex-base` /
   `aphex-website` on the next release that publishes something.
2. That workflow cuts `create-aphex-v<version>` **only if the tag doesn't exist yet**.
3. Which means the tag only moves when **`create-aphex` itself bumps**.
...
node packages/create-aphex/dist/index.js my-test-app --template website
```
