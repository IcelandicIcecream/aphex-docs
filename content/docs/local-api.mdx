...
<Callout type="warn">
	**It throws on an unauthenticated request** — `locals.auth` is `null` for an anonymous visitor,
...
// removed: organizationId, createdBy, updatedBy, publishedHash
```

Four fields go: `organizationId`, `createdBy`, `updatedBy` and `publishedHash`. The
...
`public` — see [Public reads](#public-reads) for what that strips:
