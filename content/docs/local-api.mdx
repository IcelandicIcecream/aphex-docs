...
<Callout type="warn">
	**Run `pnpm generate:types` after every schema change.** Typesafety on `localAPI.collections.<name>` is powered by generated TypeScript interfaces — without regenerating, your IDE won't see new fields or renames. Pair it with `pnpm db:push` so the runtime and the types stay in sync. See [Type-safe collections](#type-safe-collections) below for the full lifecycle.
</Callout>
...
The Local API's typesafety isn't automatic — it's powered by generated TypeScript interfaces that mirror your current schema. The lifecycle to keep types in sync:

1. **Edit a schema** in `src/lib/schemaTypes/` (add a field, rename a document type, change a reference, etc.)
2. **Run `pnpm generate:types`** — this regenerates the module augmentation file. Without this step, `localAPI.collections.<name>` will still type as `unknown` (or the previous shape), and your IDE won't catch field renames or new fields.
...
You'll typically run `pnpm generate:types` after every schema change, alongside `pnpm db:push` (or `db:generate` + `db:migrate`) which syncs the database schema. Treat them as a pair — DB migration without type generation leaves you with a working runtime but no IDE autocomplete; type generation without DB migration gives you autocomplete for fields the database doesn't yet have.

See [Type Generation](/docs/type-generation) for the underlying mechanism (module augmentation, file outputs, CI considerations).
