...
<Callout>
	...
	exactly as before — so existing code keeps working unchanged. Pass it whenever a write is based
	on data you read earlier: a second browser tab, a background job, or an AI agent editing a
	document someone has open.
</Callout>
