...
## `ve.edit(target)` — click-to-edit for content stega can't mark up

The CMS auto-encodes a document's own string fields, so most text is already clickable. `ve.edit` covers what that can't reach. Spread the returned attributes onto any element; outside preview it returns `{}`, so it's inert on the live site.

It has two targets, chosen by which keys you pass.

### Another document — `{ id, type }`

Sometimes a page renders a document it isn't "about" — a **"latest posts" block** on the home page, a related-articles list — where the items come from an app-level query, not a stored reference on the current document. Those cards have no field to encode, but they _are_ real documents with real ids:
...
Clicking opens that post's editor as a modal, keeping the page you were editing underneath.

### A field of the open document — `{ field, arrayIndex }`

Clicking reveals that field in the form pane, and with `arrayIndex`, the specific row. Use it for a list whose entries are references: **revealing the row is what lets an author reorder or remove it**, which opening the referenced document does not.

```svelte
{#each menu.items as item, i}
	<li {...ve.edit({ field: 'items', arrayIndex: i })}>{item.name}</li>
{/each}
```

### Choosing between them with `ve.documentType`

When one page is reachable from several document types, the right target depends on which one the author has open. `ve.documentType` is the schema type currently being edited, or `null` outside preview:

```svelte
{#each menu.items as item, i}
	{@const attrs =
		ve.documentType === 'menu'
			? ve.edit({ field: 'items', arrayIndex: i })
			: ve.edit({ id: item.id, type: 'menuItem', field: 'name' })}
	<li {...attrs}>{item.name}</li>
{/each}
```

Editing the menu reveals the dish's slot in the list; editing a dish — whose preview URL is also this page — opens that dish instead.

<Callout>
	The hover overlay appends `[n]` to the label for a row, so several rows of one list don't all read
	as the same bare field name.
</Callout>
...
| `usePreview()`                   | Returns `{ inPreview, document, documentType, live, encode, edit, image }`. Call once per component. |
| `ve.documentType`                | Schema type currently open in the editor, or `null` outside preview.                                 |
| `ve.edit({ id, type })`          | Attributes to make any element open a _different_ document (app-level references).                   |
| `ve.edit({ field, arrayIndex })` | Attributes to reveal a field — or one row of it — of the document being edited.                      |
