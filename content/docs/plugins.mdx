...
Register an **agent tool** and it becomes callable over [MCP](/docs/mcp) — and by the [in-admin assistant](/docs/ai-assistant) — with no app-level wiring, the same self-registration as event consumers and job handlers. A part is a `{ definition, execute }` pair:
