...
<Cards>
	<Card
		title="Install a ready-made plugin"
		href="#use-a-ready-made-plugin"
		description="Add an npm package, register it in one file, restart."
	/>
	<Card
		title="Write your own"
		href="#write-your-own-plugin"
		description="A plugin can live right inside your project — no package, no publishing."
	/>
</Cards>
...
Publish any document in the admin. Your handler runs. (Reactions run through the job worker — see [Events & Jobs](/events-and-jobs) for how to keep it running; in local dev it fires within seconds.)
...
<Callout>
	...
	including how to define and emit your **own** events, is in [Events & Jobs](/events-and-jobs).
</Callout>
...
A sidebar-placed tool lands in a group labelled **Tools** by default. Add `group` to file it under one of the app's own sidebar groups instead — the value is a `SidebarNavGroup` id (see [Admin sidebar](/configuration#admin-sidebar)):
...
```ts
{
	...
	component: ReportsTool,
	placement: 'sidebar',
	group: 'ops'                  // renders inside the app's `ops` group
}
```

Naming a group the host app hasn't defined is safe: the tool falls back to the default **Tools** group rather than disappearing, so a plugin keeps working in an app that never heard of `ops`.
...
Register a **job handler** to run work your plugin (or app) enqueues via `databaseAdapter.scheduleJob(...)` — a sync, a report, a cleanup. Where an event consumer is triggered _by an event_, a job handler runs whatever was _explicitly queued_.
...
Namespace job types (`reports.nightly`) to avoid collisions. See [Events & Jobs](/events-and-jobs).

### Give an AI agent a tool

Register an **agent tool** and it becomes callable over [MCP](/mcp) — and by the [in-admin assistant](/ai-assistant) — with no app-level wiring, the same self-registration as event consumers and job handlers. A part is a `{ definition, execute }` pair:
