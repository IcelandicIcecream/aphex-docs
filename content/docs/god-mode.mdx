...
The layout guard at <Src path="apps/studio/src/routes/god-mode/+layout.server.ts" /> rejects anyone else with an "Access Denied" page; unauthenticated requests redirect to `/login`. Treat the role as production-sensitive — it bypasses every per-org capability check.
...
The `instance_settings` table holds a single row per instance and is keyed by the `InstanceSettings` interface in <Src path="packages/cms-core/src/lib/types/instance.ts" />:
