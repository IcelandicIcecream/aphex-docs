...
The guiding rule mirrors [schema hooks](/schemas/hooks):
...
Register an [`aphex/event/consumer`](/plugins#run-code-when-a-document-is-published) part. It subscribes to one or more event types and runs when they fire:
...
- **Consumers read their own config.** `settings.get(pluginId)` returns the plugin's decrypted [settings](/plugins#give-your-plugin-a-settings-screen) for the event's organization — so a webhook URL stored as a `secret` is available inside the reaction, without threading anything through.
...
| `user.deleted`       | a user is deleted, once for each organization  | `userId`, `email`, `image`                    |
...
First-party plugins own their own event namespaces and publish their catalogs in their feature
documentation:

| Plugin                   | Event                      | Fires when                          | Payload                  |
| ------------------------ | -------------------------- | ----------------------------------- | ------------------------ |
| `@aphexcms/plugin-forms` | `forms.submission.created` | a form submission and outbox commit | `submissionId`, `formId` |

See [Forms](/forms#notifications) for its delivery behavior. Third-party plugin events follow the
same exact-match subscription model; consult that plugin's documentation for its catalog.
...
From any server code — commonly a [server route](/plugins#add-an-api-endpoint) receiving a webhook. Call `appendEvent` inside `withTransaction` so the event commits atomically with any DB write it accompanies, and `parse` the payload so a bad shape fails at the write site.
...
<TypeTable
	type={{
		workerSecret: {
			type: 'string',
			description:
				'Bearer secret gating POST /api/internal/workers/run. Unset → endpoint disabled (404).'
		},
		batchSize: { type: 'number', default: '10', description: 'Max jobs claimed per worker tick.' },
		leaseMs: {
			type: 'number',
			default: '30000',
			description: 'Lease per claimed job before it is reclaimable (crash recovery).'
		},
		relayBatchSize: {
			type: 'number',
			default: '100',
			description: 'Max outbox rows fanned out per tick.'
		},
		handlers: {
			type: 'JobHandlerMap',
			description: 'App-level job handlers, keyed by job type. Wins over plugin handlers.'
		}
	}}
/>
...
- [Plugins](/plugins) — the `aphex/event/consumer` and `aphex/job/handler` parts in full.
- [Schema hooks](/schemas/hooks) — the _transform_ half of the rule (events are the _react_ half).
- [Settings & secrets](/plugins#give-your-plugin-a-settings-screen) — how a consumer reads its own encrypted config.
